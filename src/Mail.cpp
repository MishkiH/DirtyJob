...
int Mail::CurrentWave() const {
    ...
    for (const auto& mail : mails_) {
        if ((mail.id == 1 || mail.id == 2) && mail.finished)
            finished_1_2 = true;
        if ((mail.id == 3 || mail.id == 4) && mail.finished)
            finished_3_4 = true;
        if (mail.id == 5 && mail.finished)
            finished_5 = true;
    }
    if (!finished_1_2)
        return 1;
...
std::vector<const MailEntry*> Mail::GetAvailableMails() const {
    ...
    for (const auto& mail : mails_) {
        ...
        if (wave == 1 && (mail.id == 1 || mail.id == 2)) result.push_back(&mail);
        else if (wave == 2 && (mail.id == 3 || mail.id == 4)) result.push_back(&mail);
        else if (wave == 3 && mail.id == 5) result.push_back(&mail);
    }
    return result;
}
...
void Mail::RejectOtherMailsInCurrentWave(int except_id) {
    int wave = 0;
    if (except_id == 1 || except_id == 2) wave = 1;
    else if (except_id == 3 || except_id == 4) wave = 2;
    else if (except_id == 5) wave = 3;

    for (auto& mail : mails_) {
        if (mail.id != except_id && !mail.taken && !mail.finished && !mail.rejected) {
